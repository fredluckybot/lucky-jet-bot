...
# Mémoire intelligente (Mémorise les cycles du jeu)
historique_scores = []

def analyser_tendance():
    if not historique_scores: return "🔄 ANALYSE..."
...
def generate_fair_prediction(seed):
    ...
    if pred > 35: pred = round(random.uniform(5.0, 15.0), 2)
    return pred, hash_res

@bot.message_handler(commands=['start'])
def start(m):
    bot.reply_to(m, "🤖 **MONITOR v5.0 ACTIVÉ**\nSurveillance seconde par seconde en cours...")
...
def monitor(m):
    try:
        ...
        # Effet visuel remarquable
        wait = bot.reply_to(m, "🛰️ **Mémorisation du flux...**\n`[██████░░░░]`", parse_mode="Markdown")
        time.sleep(1) 
        ...
        res = (f"🚀 **SIGNAL VALIDÉ**\n━━━━━━━━━━━━━━\n"
               ...
               f"🎯 **PRÉDICTION :** `{pred}x`\n━━━━━━━━━━━━━━\n"
               f"🔐 **HASH :** `{h[:30]}...`\n\n"
               f"📡 *Le bot mémorise le jeu chaque seconde.*")
        bot.edit_message_text(res, m.chat.id, wait.message_id, parse_mode="Markdown")
    except:
        bot.reply_to(m, "⚠️ Envoie un score (ex: 1.50)")
